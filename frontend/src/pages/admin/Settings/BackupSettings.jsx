        ...
        <div className="w-64 border-r border-slate-200 bg-slate-50/50 p-4">
            ...
            <nav className="space-y-1">
                <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeTab === 'MANUAL' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-200' : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'}`}
                >
                    <Database size={18} /> Manual Backup
                </button>
                <button
                    onClick={() => setActiveTab('SCHEDULE')}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeTab === 'SCHEDULE' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-200' : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'}`}
                >
                    <Clock size={18} /> Schedule
                </button>
                <button
                    onClick={() => setActiveTab('HISTORY')}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeTab === 'HISTORY' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-200' : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'}`}
                >
                    <History size={18} /> Backup History
                </button>
                <button
                    onClick={() => setActiveTab('RESTORE')}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeTab === 'RESTORE' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-200' : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'}`}
                >
        ...
        <div className="max-w-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-6 bg-blue-50 border border-blue-100 rounded-xl hover:shadow-md transition-shadow cursor-pointer" onClick={() => handleManualBackup('FULL')}>
                    <div className="bg-blue-100 w-10 h-10 rounded-lg flex items-center justify-center text-primary mb-4">
                        <Database size={20} />
                    </div>
        ...
        <div className="flex bg-white rounded-xl shadow-sm border border-slate-200 min-h-[600px]">
            ...
            <div className="flex-1 p-8">
                <div className="flex items-center justify-between mb-8">
                    ...
                    {loading && <div className="flex items-center gap-2 text-primary text-sm font-medium animate-pulse"><RotateCcw className="animate-spin" size={16} /> Processing...</div>}
                </div>
