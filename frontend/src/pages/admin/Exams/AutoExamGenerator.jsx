...
const AutoExamGenerator = () => {
    ...
    // Structure & Marking
    const [qStructure, setQStructure] = useState({ mcq: 0, cq: 0, short: 0 });
    const marksConfig = { MCQ: 1, CQ: 10, SHORT: 2 };
    ...
    // Checks & Balances
    const totalMarks = (qStructure.mcq * marksConfig.MCQ) + (qStructure.cq * marksConfig.CQ) + (qStructure.short * marksConfig.SHORT);
    const totalQuestions = qStructure.mcq + qStructure.cq + qStructure.short;
    ...
    const handleGenerate = async () => {
        ...
        try {
            const payload = {
                ...
                questionTypeRules: [
                    { questionType: 'MCQ', count: qStructure.mcq, marksPerQuestion: marksConfig.MCQ },
                    { questionType: 'CQ', count: qStructure.cq, marksPerQuestion: marksConfig.CQ },
                    { questionType: 'SHORT', count: qStructure.short, marksPerQuestion: marksConfig.SHORT }
                ].filter(r => r.count > 0)
            };

            const res = await examService.generateExam(payload);
            if (res.success) {
                navigate(`/exams/generate/editor/${res.data.id}`);
            }
        } catch (e) {
            console.error(e);
                        ...
                        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 sticky top-4 overflow-hidden">
                            ...
                            <div className="p-6">
                                ...
                                <div className="space-y-4 mb-8">
                                    {[
                                        { key: 'mcq', label: 'MCQ (বহুনির্বাচনি)', mark: marksConfig.MCQ, color: 'indigo' },
                                        { key: 'cq', label: 'CQ (সৃজনশীল)', mark: marksConfig.CQ, color: 'purple' },
                                        { key: 'short', label: 'Short (সংক্ষিপ্ত)', mark: marksConfig.SHORT, color: 'teal' }
                                    ].map(type => (
                                        <div key={type.key} className="flex justify-between items-center group bg-slate-50 p-3 rounded-xl border border-slate-100">
                                            <div>
                                                <p className="font-bold text-sm text-slate-800">{type.label}</p>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase">Per Q: {type.mark} Marks</p>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <input
                                                    type="number"
                                                    min="0"
                                                    value={qStructure[type.key]}
                                                    onChange={(e) => setQStructure({ ...qStructure, [type.key]: parseInt(e.target.value) || 0 })}
                                                    className={`w-16 h-10 text-center font-black text-lg bg-white border-2 border-${type.color}-200 rounded-lg focus:border-${type.color}-500 outline-none`}
                                                />
                                            </div>
                                        </div>
                                    ))}
