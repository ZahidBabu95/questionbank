...
const invalidateExamCache = (id) => {
    if (id) {
        ...
        try {
            sessionStorage.removeItem('exam_cache_' + id);
        } catch (e) {}
    }
};
...
const deduplicatedGetKnowledge = () => {
    ...
    try {
        const stored = sessionStorage.getItem('knowledge_cache');
        if (stored) {
            const parsed = JSON.parse(stored);
    ...
    knowledgePromise = axios.get('/v1/support/knowledge').then(res => {
        ...
        try {
            sessionStorage.setItem('knowledge_cache', JSON.stringify(res));
        } catch (e) {}
...
export const useExamManager = () => {
    ...
    const handleSaveDocument = async () => {
        ...
        try {
            ...
            if (id) {
                ...
                if (res?.data) {
                    setExamCache(id, res.data);
                }
                if (setAutoSaveStatus) setAutoSaveStatus('saved');
    ...
    // --- Debounced Silent Auto-Save Engine ---
    const isInitialAutoSaveMounted = useRef(false);
    const autoSaveTimerRef = useRef(null);

    useEffect(() => {
        if (!id || !isEditorLoaded) return;

        // Skip initial trigger when document content is loaded first time
        if (!isInitialAutoSaveMounted.current) {
            isInitialAutoSaveMounted.current = true;
            return;
        }
        ...
        autoSaveTimerRef.current = setTimeout(async () => {
            try {
                ...
                console.warn("[AutoSave] Silent auto-save failed:", err);
                if (setAutoSaveStatus) {
    ...
    }, [rawContent, docSettings, id, isEditorLoaded, editorMode, setAutoSaveStatus]);
          ...
          try {
              ...
              for (let i = 0; i < actualPageCount; i++) {
                  ...
                  const canvas = await html2canvas(pages[i], {
                      scale: 3, // 300 DPI Ultra Sharp 1:1 Pixel-Perfect Resolution
                      useCORS: true,
                      allowTaint: false,
    ...
    const handleDownloadVectorPdf = async () => {
        ...
        setDownloadStatus(uiLang === 'bn' ? "ভেক্টর রেন্ডারিং ইঞ্জিন সক্রিয় হচ্ছে (Puppeteer Vector PDF)..." : "Initializing Puppeteer Vector Engine...");

        try {
            // 1. Silent save current document first
            const parsedMins = parseDurationToMinutes(docSettings.time);
            const payload = {
                title: docSettings.exam || "Nexus Exam",
                examCode: "NEXUS-" + Math.floor(Math.random() * 10000),
            ...
            setDownloadStatus(uiLang === 'bn' ? "ব্যাকএন্ডে উচ্চ-মানের ভেক্টর PDF তৈরি হচ্ছে..." : "Generating high-quality Vector PDF on backend...");

            const pageSize = docSettings.pageSize || 'A4';
            const orientation = docSettings.orientation || 'portrait';

            const response = await axios.get(`/v1/exams/manual/${id}/pdf/vector`, {
                params: { pageSize, orientation },
                responseType: 'blob'
            });

            setDownloadProgress(85);
            setDownloadStatus(uiLang === 'bn' ? "PDF প্রস্তুত! ডাউনলোড শুরু হচ্ছে..." : "PDF ready! Starting download...");

            const blob = new Blob([response.data], { type: 'application/pdf' });
            const downloadUrl = window.URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = downloadUrl;
            const fileName = (docSettings.exam || "Nexus_Exam").replace(/[^a-zA-Z0-9\u0980-\u09FF_-]/g, '_') + "_Vector.pdf";
            ...
            console.error("Vector PDF Export failed, falling back to Canvas Engine:", err);
            alert(uiLang === 'bn' ? "ভেক্টর পিডিএফ তৈরিতে সমস্যা হয়েছে। ক্যানভাস ইঞ্জিন ব্যবহার করে ডাউনলোড শুরু হচ্ছে।" : "Vector PDF generation failed. Falling back to Canvas engine.");
    ...
    const handleNativeVectorPdf = () => {
        if (!id) {
            alert(uiLang === 'bn' ? "দয়া করে প্রথমে ডকুমেন্টটি সেভ করুন।" : "Please save the document first.");
            return;
        }
        const token = localStorage.getItem('token');
        const printUrl = `/exams/print-view/${id}?token=${encodeURIComponent(token || '')}&autoPrint=true`;
        window.open(printUrl, '_blank');
    };

    return {
        templates, loadingTemplates, isSavingTemplate, isDownloadingPdf, handleDownloadPdf, handleDownloadVectorPdf, handleNativeVectorPdf,
