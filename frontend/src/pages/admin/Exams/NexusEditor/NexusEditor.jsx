...
const NexusEditor = () => {
    ...
    useEffect(() => {
            ...
            if (importedStr && !id) {
                try {
                    ...
                    if (parsed.actionable_type === 'questions' && parsed.data) {
                        ...
                        // Ask for name and save automatically
                        setTimeout(async () => {
                            const title = window.prompt(uiLang === 'bn' ? "প্রশ্নপত্রের একটি নাম দিন (যেমন: মডেল টেস্ট ১):" : "Enter a title for this document:");
                            if (title && title.trim() !== '') {
                                try {
                                    // Make sure we have a fallback subject ID if not provided by AiWorkspace (UUID format)
                                    // 00000000-0000-0000-0000-000000000000 is a safe fallback if backend allows, or we can just pass parsed.classSubjectId.
                                    // Wait, if it's missing, it will still fail, but we hope it's not missing.
        ...
        importFromAi();
    }, [id, docSettings, navigate, uiLang]);
                        ...
                        if (res.data.docSettingsJson) {
                            try {
                                ...
                                if (dynamicSections.length > 0) {
                                    parsedSettings.sections = [...(parsedSettings.sections || []), ...dynamicSections];
                                }
