...
const PrivacyPolicy = () => {
    ...
    useEffect(() => {
        ...
        const fetchPrivacy = async () => {
            try {
                const data = await cmsService.getPublicSection('PRIVACY_SECTION');
                setPrivacy(data);
    ...
    const getTranslationValue = (itemKey) => {
        if (!privacy) return '';
        const item = privacy.contents.find(c => c.contentKey === itemKey);
    ...
    const pageTitle = getTranslationValue('TITLE') || (currentLang === 'bn' ? 'গোপনীয়তা নীতিমালা' : 'Privacy Policy');
                ...
                <div className="flex-grow">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-40 space-y-4">
                            ...
                            <p className="text-slate-400 text-sm font-medium">{currentLang === 'bn' ? 'তথ্য লোড হচ্ছে...' : 'Loading secure policy...'}</p>
                        </div>
                    ) : (
                        <motion.article 
                            ...
                            <div className="prose prose-slate max-w-none prose-headings:text-slate-900 prose-p:text-slate-600 prose-p:leading-relaxed leading-relaxed font-normal">
                                {renderMarkdown(bodyContent) || (
                                    <p className="text-slate-400 italic text-center py-20">
                                        {currentLang === 'bn' ? 'গোপনীয়তা নীতিমালা উপলব্ধ নেই।' : 'No privacy policy available.'}
                                    </p>
                                )}
                            </div>
                        </motion.article>
