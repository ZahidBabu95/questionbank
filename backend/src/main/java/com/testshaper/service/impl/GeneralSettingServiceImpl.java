...
public class GeneralSettingServiceImpl implements GeneralSettingService {
    ...
    @Override
    @Cacheable(value = "aiSettings", key = "'global_' + #category.name()")
    ...
    @Override
    @Transactional
    @CacheEvict(value = "aiSettings", allEntries = true)
    ...
    private Map<String, String> mapSettingsToMap(List<GeneralSetting> settings) {
        ...
        for (GeneralSetting s : settings) {
            ...
            // Correct local file paths for branding or other URLs
            if (s.getKey().toLowerCase().endsWith("_url") && val != null && !val.startsWith("http")
                    && !val.startsWith("/")) {
                val = "/api/v1/public/files/" + val;
            }
            ...
            map.put(s.getKey().toLowerCase(), val);
        }
        return map;
    }
